...
def capitalizeFirst_extend(arr_sentence): # in case a sentence is given instead of list of words.
    arr = arr_sentence.split()
    res = []
    if len(arr) == 0:
        return res
    res.append(arr[0][0].upper() + arr[0][1:])
...
def collectStrings(obj):
    ''' function that takes an object and returns sub object with string values
        function takes input as object (dictionary) and returns an array so we have to create empty array
    '''
    res = []
    for ke in obj:
        if type(obj[ke]) is dict :
